...
**다음 알람 1회 건너뛰기**는 현재 예약된 한 번만 취소하고 그다음 선택 요일로 예약을 교체합니다. 건너뛴 시각과 새 다음 시각은 화면에 표시되며, 이 상태는 재부팅 뒤에도 유지됩니다. 이후 반복은 정상적으로 계속됩니다. **지금 알람 테스트**는 저장된 반복 일정, 건너뛰기 상태, 다음 예약을 변경하지 않습니다.
